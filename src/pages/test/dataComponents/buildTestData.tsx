...
export function buildTestSelectionStructure(
  ...
  materializations.forEach((materialization: any) => {
    ...
    relevantColumns.forEach((column: any) => {
      ...
      columnTestConfigs.push({
        ...
        testConfigs: allowedTests.map((element: any): TestSuiteConfig => {
          const typeSpecificSuite = suites.find(
            (el: any) => el.type === element
          );
    ...
    const testDefinitionSummary: TestDefinitionSummary[] = [
      {
        ...
        totalCount: columnTestConfigs.filter(
          (config) =>
            !!config.testConfigs.filter((el) => el.type === 'ColumnCardinality')
              .length
        ).length,
      },
      {
      ...
      },
      {
        ...
        totalCount: columnTestConfigs.filter(
          (config) =>
            !!config.testConfigs.filter((el) => el.type === 'ColumnUniqueness')
              .length
        ).length,
      },
    ];
    ...
    const uniqueCronValues = Array.from(
      new Set(
        columnTestConfigs.filter((el) => el.testsActivated).map((el) => el.cron)
      )
    );

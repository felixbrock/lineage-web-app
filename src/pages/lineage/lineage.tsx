...
import MetricsGraph, {
  ...
  defaultNullnessData,
  effectiveRateNullData,
  effectiveRateFreshSampleData,
  effectiveRateDistData,
  defaultDistData,
...
export default (): ReactElement => {
  ...
  const [showIntegrationSidePanel, setShowIntegrationSidePanel] =
    useState<boolean>();
  const [slackAccessToken] = useState<string>('');
  ...
  const sideNavClickCallback = async (
    ...
    */
      ...
      if (selectedEl.type === 'combo') {
        localSourceData.cols = defaultColumns.filter(
          (el) => el.materializationId === selectedEl.id
        );

        setSourceData({ ...localSourceData });

        localSourceData.dependencies = defaultDependencies.filter((el) =>
        localSourceData.cols.some(
          (col) => col.id === el.headId || col.id === el.tailId
        )
      );
        localSourceData.mats =
          defaultMaterializations.filter((el) =>
            defaultColumns.some(
              (col) =>
                col.materializationId === el.id &&
                sourceData.dependencies.some(
                  (dep) => dep.tailId === col.id || dep.headId === col.id
                )
            )
          );
        localSourceData.dashboards = defaultDashboards.filter((el) =>
          defaultColumns.some(
            (col) =>
              col.id === el.columnId &&
              sourceData.dependencies.some(
        ...
        localSourceData.cols.concat(
          defaultColumns.filter(
            (col) =>
              localSourceData.mats.some(
                (mat) => mat.id === col.materializationId
              ) ||
              localSourceData.dashboards.some(
                (dash) => dash.columnId === col.id
              )
          )
        );
      }

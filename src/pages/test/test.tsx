...
export default (): ReactElement => {
  ...
  const handleMatFrequencyChange = (event: any) => {
    ...
    testSelectionLocal[props[1]].columnTestConfigs.forEach((el, index) => {
  ...
  const handleMatSensitivityChange = (event: any) => {
    ...
    testSelectionLocal[props[1]].columnTestConfigs.forEach((el, index) => {
      ...
      testSelectionLocal[props[1]].columnTestConfigs[index].sensitivity = value;
    });
  ...
  const handleTestSelectButtonClick = async (event: any) => {
    ...
    if (!testSuiteId) {
      ...
      testSelectionLocal[props[1]].columnTestConfigs[
        columnTestConfigIndex
      ].testConfigs[testConfigIndex].testSuiteId = testSuite.id;
  ...
  const handleMatLevelColumnTestButtonClick = async (event: any) => {
    ...
    await Promise.all(
      testSelectionLocal[props[1]].columnTestConfigs.map(
        async (config, index) => {
